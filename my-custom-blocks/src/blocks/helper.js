...
// Diagonal line on no colour (using ::before via inline style object workaround)
        ...
        {() => (
          <div>
            ...
            <div>
              ...
              >
                {__('Transparency')} ({100 - alpha}% transparent)
              </label>
              <input
                type="range"
                min={0}
...
const UnitInputControl = ({
  ...
  <div style={{ display: 'grid', gap: '0.5em', marginBottom: '0.25em' }}>
    <label
      style={{ fontWeight: '600', display: 'block', marginBottom: '0.25em' }}
    >
      {label}
    </label>
...
// Custom tabs panel with standard tabs (general, styles, advanced)
const CustomTabPanel = ({
  ...
  // Spacing options for padding and margin (Kadence global variables)
  const spacingOptions = [
    { label: __('Default'), value: '' },
    {
      label: 'XXS (--global-kb-spacing-xxs)',
      value: 'var(--global-kb-spacing-xxs, var(--global-xxs-spacing, 0.5em))',
    },
    {
      label: 'XS (--global-kb-spacing-xs)',
      value: 'var(--global-kb-spacing-xs, var(--global-xs-spacing, 1em))',
    },
    {
      label: 'SM (--global-kb-spacing-sm)',
      value: 'var(--global-kb-spacing-sm, var(--global-sm-spacing, 1.5em))',
    },
    {
      label: 'MD (--global-kb-spacing-md)',
      value: 'var(--global-kb-spacing-md, var(--global-md-spacing, 2em))',
    },
    {
      label: 'LG (--global-kb-spacing-lg)',
      value: 'var(--global-kb-spacing-lg, var(--global-lg-spacing, 2.5em))',
    },
    {
      label: 'XL (--global-kb-spacing-xl)',
      value: 'var(--global-kb-spacing-xl, var(--global-xl-spacing, 3.5em))',
    },
    {
      label: '2XL (--global-kb-spacing-xxl)',
      value: 'var(--global-kb-spacing-xxl, var(--global-xxl-spacing, 5em))',
    },
  ];
  ...
  const renderSpacingControls = (label, type) => {
    const isLinked = attributes[`isLinked${type}`] ?? true;
    const useCustom = attributes[`useCustom${type}`] ?? false;

    return (
      <BaseControl label={__(label)}>
        <ToggleControl
          label={__('Link All Sides')}
          checked={isLinked}
          onChange={(val) => setAttributes({ [`isLinked${type}`]: val })}
        />
        <ToggleControl
          label={__('Use Custom Sizes')}
          checked={useCustom}
          onChange={(val) => setAttributes({ [`useCustom${type}`]: val })}
        />

        {isLinked ? (
          useCustom ? (
            <div className="rs-spacing-custom-wrapper">
              <TextControl
                label={__('All Sides')}
                type="number"
                value={attributes[`${type.toLowerCase()}All`] || ''}
                onChange={(val) => setAttributes({ [`${type.toLowerCase()}All`]: val })}
              />
              <SelectControl
                label={__('Unit')}
                value={attributes[`${type.toLowerCase()}Unit`] || 'px'}
                options={unitOptions}
                onChange={(val) => setAttributes({ [`${type.toLowerCase()}Unit`]: val })}
              />
            </div>
          ) : (
            <SelectControl
          ...
          ['Top', 'Right', 'Bottom', 'Left'].map((side) => {
            ...
            return useCustom ? (
              <div key={key} className="rs-spacing-custom-wrapper">
                <TextControl
                  label={__(`${label} ${side}`)}
                  type="number"
                ...
                <SelectControl
                  label={__('Unit')}
                  value={attributes[unitKey] || 'px'}
                  options={unitOptions}
                  onChange={(val) => setAttributes({ [unitKey]: val })}
                />
              </div>
            ) : (
              <SelectControl
  ...
  return (
    <TabPanel
      ...
      tabs={[
        {
          name: 'general',
          title: <>{blockDefault} {__('General')}</>,
          className: 'tab-general',
        },
        {
          name: 'styles',
          title: <>{brush} {__('Styles')}</>,
          className: 'tab-styles',
        },
        {
          name: 'advanced',
          title: <>{cog} {__('Advanced')}</>,
        ...
        switch (tab.name) {
          ...
          case 'advanced':
            return (
              <>
                <PanelBody
                  className={'rs-block-margin-padding-panel'}
                  title={__('Spacing')}
                  initialOpen={true}
                >
                  {renderSpacingControls('Padding', 'Padding')}
                  {renderSpacingControls('Margin', 'Margin')}
                </PanelBody>
                <PanelBody
                  className={'rs-block-advanced-panel'}
